...
Default camera symlinks (used by `start_gripper.cpp`):

| Side | Cameras |
|------|---------|
| Left | `/dev/left_video_0_main`, `/dev/left_video_1_main`, `/dev/left_video_2_main` |
| Right | `/dev/right_video_0_main`, `/dev/right_video_1_main`, `/dev/right_video_2_main` |
...
Build the project first, then query the device MCUID. Do **not** run `start_gripper.cpp` at the same time.
