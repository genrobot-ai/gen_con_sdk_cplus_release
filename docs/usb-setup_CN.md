...
默认相机软链接（`start_gripper.cpp` 使用）：

| 侧 | 相机 |
|---|------|
| 左 | `/dev/left_video_0_main`、`/dev/left_video_1_main`、`/dev/left_video_2_main` |
| 右 | `/dev/right_video_0_main`、`/dev/right_video_1_main`、`/dev/right_video_2_main` |
...
先完成编译，再查询设备 MCUID。**不要**同时运行 `start_gripper.cpp`。
